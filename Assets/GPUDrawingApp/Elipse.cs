...
public class Elipse : MonoBehaviour
{
    ...
    [SerializeField] ElipseSpawner _targetSpawner;
    ...
    void Update()
    {
        ...
        if (_targetResetTimer < 0)
        {
            _targetResetTimer = _targetResetIntervalSeconds;
            _targetSpawner.ClearAll(false);
            _targetSpawner.Spawn(Screen.width / 2 - 100, Screen.height / 2 - 100, 200, 200, _targetSize); // TODO refactor this duplicate code
            ...
            (Vector3.Distance(_strokeStartPos, _targetPos2) < _targetSize && Vector3.Distance(_strokeEndPos, _targetPos1) < _targetSize))
        {
            ...
            ClearScreen();
            _targetSpawner.Spawn(Screen.width / 2 - 100, Screen.height / 2 - 100, 200, 200, _targetSize);
            //List<Target> targets = _targetSpawner.Spawn(Screen.width / 2 - 100, Screen.height / 2 - 100, 200, 200, _targetSize);
            //_targetPos1 = targets[0].GetComponent<RectTransform>().position;
            //_targetPos2 = targets[1].GetComponent<RectTransform>().position;
